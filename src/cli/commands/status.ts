...
export const statusCommand = new Command("status")
  ...
  .action((options) => {
    ...
    if (sessions.length === 0) {
      ...
      console.log("  Run \x1b[36mstep-zero review\x1b[0m or \x1b[36msz review\x1b[0m to start a new review.");
      console.log();
      return;
    }
    ...
    for (const session of sessions) {
      ...
      const openThreads = session.threads.filter((t) => t.status === "open").length;
      const totalComments = session.threads.reduce((sum, t) => sum + t.comments.length, 0);

      console.log(`  \x1b[1m${session.branch}\x1b[0m → \x1b[90m${session.baseBranch}\x1b[0m`);
      console.log(
